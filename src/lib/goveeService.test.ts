...
describe("GoveeService", () => {
  ...
  it("should add device on scan message", () => {
    ...
    (service as any).onUdpMessage(buf, {
      address: "1.2.3.4",
      port: 1234,
    } as any);
  ...
  it("should log devStatus if extendedLogging is true", () => {
    ...
    expect(logger.info.calledWithMatch("device status message data")).to.be
      .true;
  });
  ...
  describe("Command Sending", () => {
    ...
    it("should send color command with correct RGB format", () => {
      ...
      const expectedMessage = {
        msg: { cmd: "colorwc", data: { color: { r: 255, g: 0, b: 0 } } },
      };
      const sendSpy = (service as any).socket.send as sinon.SinonStub;
  ...
  describe("State Change Handling", () => {
    ...
    it("should handle onOff state change", () => {
      ...
      service.handleStateChange(
        "govee-local.0.device.devStatus.onOff",
        state,
        "192.168.1.100",
      );

      expect(
        (service.sendTurnCommand as sinon.SinonStub).calledWith(
          "192.168.1.100",
          true,
        ),
      ).to.be.true;
    ...
    it("should handle brightness state change", () => {
      ...
      service.handleStateChange(
        "govee-local.0.device.devStatus.brightness",
        state,
        "192.168.1.100",
      );

      expect(
        (service.sendBrightnessCommand as sinon.SinonStub).calledWith(
          "192.168.1.100",
          80,
        ),
      ).to.be.true;
    ...
    it("should handle color state change", () => {
      ...
      service.handleStateChange(
        "govee-local.0.device.devStatus.color",
        state,
        "192.168.1.100",
      );

      expect(
        (service.sendColorCommand as sinon.SinonStub).calledWith(
          "192.168.1.100",
          "#00FF00",
        ),
      ).to.be.true;
    ...
    it("should handle color temperature state change", () => {
      ...
      service.handleStateChange(
        "govee-local.0.device.devStatus.colorTemInKelvin",
        state,
        "192.168.1.100",
      );

      expect(
        (service.sendColorTempCommand as sinon.SinonStub).calledWith(
          "192.168.1.100",
          4000,
        ),
      ).to.be.true;
  ...
  describe("Error Handling", () => {
    it("should handle malformed JSON in UDP messages", () => {
      ...
      expect(() => {
        (service as any).onUdpMessage(malformedMessage, {
          address: "1.2.3.4",
          port: 1234,
        });
      }).to.throw();
  ...
  describe("Manual IP Configuration", () => {
    ...
    it("should request device status for manual devices", () => {
      ...
      expect(
        (service.requestDeviceStatus as sinon.SinonStub).calledWith(
          "192.168.1.50",
        ),
      ).to.be.true;
      expect(
  ...
  describe("Edge Cases - Command Boundary Values", () => {
    ...
    describe("Brightness Boundary Values", () => {
      it("should send brightness command with minimum value (0)", () => {
        ...
        expect(sendSpy.calledOnce).to.be.true;
        const sentBuffer = sendSpy.getCall(0).args[0];
        expect(JSON.parse(sentBuffer.toString())).to.deep.equal(
          expectedMessage,
        );
      });
      ...
      it("should send brightness command with float values", () => {
        ...
        const expectedMessage = {
          msg: { cmd: "brightness", data: { value: 50.5 } },
        };
        const sendSpy = (service as any).socket.send as sinon.SinonStub;
    ...
    describe("Color Boundary Values", () => {
      ...
      it("should send color command with pure green (#00FF00)", () => {
        ...
        const expectedMessage = {
          msg: { cmd: "colorwc", data: { color: { r: 0, g: 255, b: 0 } } },
        };
        const sendSpy = (service as any).socket.send as sinon.SinonStub;
      ...
      it("should send color command with pure blue (#0000FF)", () => {
        ...
        const expectedMessage = {
          msg: { cmd: "colorwc", data: { color: { r: 0, g: 0, b: 255 } } },
        };
        const sendSpy = (service as any).socket.send as sinon.SinonStub;
      ...
      it("should send color command with black (#000000)", () => {
        ...
        const expectedMessage = {
          msg: { cmd: "colorwc", data: { color: { r: 0, g: 0, b: 0 } } },
        };
        const sendSpy = (service as any).socket.send as sinon.SinonStub;
      ...
      it("should send color command with white (#FFFFFF)", () => {
        ...
        const expectedMessage = {
          msg: { cmd: "colorwc", data: { color: { r: 255, g: 255, b: 255 } } },
        };
        const sendSpy = (service as any).socket.send as sinon.SinonStub;
