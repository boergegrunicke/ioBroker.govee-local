...
export class GoveeService extends EventEmitter {
  ...
  static readonly scanMessage = {
    msg: { cmd: "scan", data: { account_topic: "reserved" } },
  };
  static readonly requestStatusMessage = {
  ...
  private serverBound(): void {
    ...
    if (
      ...
      this.options.manualIpAddresses.length > 0
    ) {
    ...
    if (this.scanMode === "interval") {
      this.searchInterval = setInterval(
        () => this.sendScan(),
        this.options.searchInterval * 1000,
      );
    ...
    switch (messageObject.msg.cmd) {
      case "scan": {
        if (messageObject.msg.data.device) {
          ...
          if (
            this.options.extendedLogging &&
            !this.loggedDevices.includes(remote.address.toString())
          ) {
        ...
        if (sendingDevice) {
          if (this.options.extendedLogging) {
            ...
            if (this.lastStatusLog[remote.address] !== statusString) {
              this.options.logger?.info(
                `device status message data: ${statusString}`,
              );
              this.lastStatusLog[remote.address] = statusString;
            }
          }
          this.emitDeviceStatusUpdate(
            sendingDevice,
            remote.address,
            messageObject,
          );
      ...
      default: {
        this.options.logger?.debug(
          `message from: ${remote.address}:${remote.port} - ${message.toString()}`,
        );
      }
    }
  }
  ...
  public requestDeviceStatus(receiver: string): void {
    const requestDeviceStatusBuffer = Buffer.from(
      JSON.stringify(GoveeService.requestStatusMessage),
    );
    this.socket.send(
  ...
  public sendScan(): void {
    ...
    const scanMessageBuffer = Buffer.from(
      JSON.stringify(GoveeService.scanMessage),
    );
    this.socket.send(
      scanMessageBuffer,
      0,
  ...
  public addManualDevices(ipAddresses: string[]): void {
    for (const ip of ipAddresses) {
      ...
      if (!isValidIpAddress(trimmedIp)) {
        this.options.logger?.error(
          `Invalid IP address format: "${trimmedIp}" - skipping this entry`,
        );
        continue;
      }
      ...
      this.options.logger?.info(
        `Added manual device: ${deviceName} at ${trimmedIp}`,
      );
  ...
  public sendBrightnessCommand(receiver: string, value: number): void {
    ...
    const brightnessMessageBuffer = Buffer.from(
      JSON.stringify(brightnessMessage),
    );
    this.socket.send(
   ...
   */
  private emitDeviceStatusUpdate(
    deviceName: string,
    ip: string,
    messageObject: any,
  ): void {
