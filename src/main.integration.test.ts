...
describe("GoveeLocal Integration Tests", () => {
  ...
  it("should handle device discovery events", async () => {
    ...
    expect(
      updateStateStub.calledWith("TestLamp.deviceInfo.ip", "192.168.1.100"),
    ).to.be.true;
  });
  ...
  it("should handle device status update events", async () => {
    ...
    expect(setObjectStub.calledWith("TestLamp.devStatus.brightness")).to.be
      .true;
    expect(setObjectStub.calledWith("TestLamp.devStatus.color")).to.be.true;
    expect(setObjectStub.calledWith("TestLamp.devStatus.colorTemInKelvin")).to
      .be.true;

    expect(updateStateStub.calledWith("TestLamp.devStatus.onOff", true)).to.be
      .true;
    expect(updateStateStub.calledWith("TestLamp.devStatus.brightness", 80)).to
      .be.true;
    expect(updateStateStub.calledWith("TestLamp.devStatus.color", "#FF0000")).to
      .be.true;
    expect(
      updateStateStub.calledWith("TestLamp.devStatus.colorTemInKelvin", 3000),
    ).to.be.true;
  });
  ...
  it("should handle state changes and forward to service", async () => {
    ...
    (adapter as any).setObjectNotExists = sinon.stub();
    (adapter as any).subscribeStates = sinon.stub();
    (adapter as any).getStateAsync = sinon
      .stub()
      .resolves({ val: "192.168.1.100" });

    // Initialize service
    await (adapter as any).onReady();

    // Mock the service method
    const handleStateChangeSpy = sinon.spy(
    ...
    await (adapter as any).onStateChange(
      "govee-local.0.TestLamp.devStatus.onOff",
      stateChange,
    );

    expect(handleStateChangeSpy.calledOnce).to.be.true;
    expect(handleStateChangeSpy.getCall(0).args[0]).to.equal(
      "govee-local.0.TestLamp.devStatus.onOff",
    );
  ...
  it("should clean up service on unload", () => {
    ...
    expect((adapter as any).goveeService.removeAllListeners.calledOnce).to.be
      .true;
