...
describe("getDatapointDescription", () => {
  it("should return correct descriptions for known keys", () => {
    expect(getDatapointDescription("model")).to.equal(
      "Specific model of the Lamp",
    );
    expect(getDatapointDescription("ip")).to.equal("IP address of the Lamp");
    expect(getDatapointDescription("bleVersionHard")).to.equal(
      "Bluetooth Low Energy Hardware Version",
    );
    expect(getDatapointDescription("bleVersionSoft")).to.equal(
      "Bluetooth Low Energy Software Version",
    );
    expect(getDatapointDescription("wifiVersionHard")).to.equal(
      "WiFi Hardware Version",
    );
    expect(getDatapointDescription("wifiVersionSoft")).to.equal(
...
describe("GoveeLocal", () => {
  it("should construct without error and have config fields", () => {
    ...
    expect(instance.config.scanMode).to.equal("interval");
  });
});
